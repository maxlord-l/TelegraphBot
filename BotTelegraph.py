...
async def send_welcome(message: types.Message):
    ...
    await message.reply("Hi!\nI'm TelegraphBot, i'll send you forecast\n"
                        ...
                        "Список команд: '/Санкт-Петербург', '/start', '/help'")

@dp.message_handler(commands=['Санкт-Петербург'])
async def get_weather_spb(message: types.Message):
    code_to_icon = {
    ...
    try:
        res = requests.get(
            f'http://api.openweathermap.org/data/2.5/weather?q=Saint Petersburg&appid={weather_key}&units=metric'
        )
        data = res.json()
